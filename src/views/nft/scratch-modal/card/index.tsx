...
const CardContent = ({ data, config, type, from }: any) => {
  ...
  return (
    ...
    >
      ...
      <Rare rare={data?.rare || 5} id={data.id} from={from} />

      {from === "detail" && (
        <div className="absolute bottom-0 left-0 z-[22] h-[26px] px-[24px] flex items-center justify-between w-full text-[10px] font-light">
          <div>Owner: {data?.address}</div>
          <a
            className="flex items-center gap-[4px] button hover:underline"
            href={`https://magiceden.io/item-details/berachain/${data.reward_token_info?.[0]?.address}/${data.reward_token_info?.[0]?.token_id}`}
            target="_blank"
          >
      ...
      >
        <div className="flex justify-between items-center mt-[6px]">
          ...
          <div className="flex gap-[4px] items-center">
            ...
            >
              {data?.total_prize}/{data.reward_token_price?.[0]?.last_price}
            </div>
            <img
