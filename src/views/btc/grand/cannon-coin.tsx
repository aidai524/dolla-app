...
const CannonCoin = forwardRef<any, any>(
  ({ scene, world, coin, onFlipComplete, isFlipping }, ref) => {
    ...
    useEffect(() => {
      ...
      scene.add(coinMesh);
      const coinShape = new CANNON.Cylinder(2, 2, 0.4, 64);
    ...
    const updateCoin = () => {
      ...
      if (isAlmostStopped && coinBody.position.z < 0) {
        ...
        if (!physics.stablePosition) {
          ...
          if (stableElapsed > 300) {
            ...
            if (distance > 0.3) {
              ...
              coinBody.position.x +=
                (physics.stablePosition.x - coinBody.position.x) * pullStrength;
              coinBody.position.y +=
                (physics.stablePosition.y - coinBody.position.y) * pullStrength;
            }
          }
        }
      } else {
    ...
    // Flip coin animation
    const flipCoin = (_flipResult: "heads" | "tails") => {
      if (!coinBodyRef.current || !coinMeshRef.current || isFlipping) {
        return;
      }
      setFlipResult(_flipResult);
      const coinMesh = coinMeshRef.current;
      const coinBody = coinBodyRef.current;
    ...
    useImperativeHandle(ref, () => ({
      flipCoin,
      // Function to update flip result during the flip animation
      updateFlipResult: (newResult: "heads" | "tails") => {
