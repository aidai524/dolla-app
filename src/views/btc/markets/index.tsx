...
export default function BTCMarkets({ ref, onSelectMarket }: any) {
  ...
  const { containerRef, isLoading } = useInfiniteScroll(onQueryPoolList, {
    loading,
    hasMore,
    threshold: 100,
    debounceWait: 300
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed z-[20] top-[75px] right-0 w-[332px] h-[calc(100vh-75px)] bg-[#1A1E24] border border-[#2B2B2B]"
          >
            <div className="flex items-center justify-between px-[24px] pt-[20px]">
              ...
              {Filters.map((filter) => (
                <button
                  className={clsx(
                    "button p-[8px] text-[12px] text-[#ADBCCF] border border-[#383F47] rounded-[6px]",
                    sortField === filter.key ? "bg-[#2E353F]" : "bg-[#1A1E24]"
                  )}
                  key={filter.key}
                  onClick={() => setSortField(filter.key)}
                >
                  {filter.label}
                </button>
              ))}
            ...
            <div
              ref={containerRef}
              className="h-[calc(100%-56px)] overflow-y-auto px-[24px] pt-[8px] overflow-x-hidden"
            >
              {poolList.map((market) => (
                <Market
                  key={market.id}
